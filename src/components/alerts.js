...
const Alert = (props) => {
  ...
  return (
    <div data-index={props.index} data-type={props.type} data-value={props.value} className="alert" style={{ top: fromTop + "em" }} >
...
export default function AlertList(props) {
  ...
  const alertsList = props.alertsList.length <= 0 ? [] : props.alertsList.slice(0, 7).map((alert, i) => {
    ...
    return (
      <Alert key={i} index={i} type={alert.type} value={value} message={alert.message} close={alert.close} confirm={alert.confirm} handleClick={handleClick} />
    )
  });
